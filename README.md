...
- **吟味で締める**: 本文(3行)を書き終えたら、最後に **吟味コメント** を1本投稿してからcloseする。吟味の3問= **クリティカルか**(この問題が未解決だと誰が何に困るか・なぜ非自明か)/ **手段として適切か**(設計が測るものは問題意識と一致しているか)/ **答え切ったか**(主張の強さは証拠に見合うか)。書式は各問を `###` 見出しにし、答えを本文に書く(問いと答えを同じ大きさで並べない)。
